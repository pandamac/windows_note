...
LRESULT CALLBACK KeyboardProc(int code,WPARAM wParam,LPARAM lParam)
{
	...
	{
		myMsg=(MSG *)lParam;
		if (myMsg->message==WM_KEYDOWN || myMsg->message==WM_KEYUP || myMsg->message==WM_CHAR || myMsg->message==WM_SYSCHAR || myMsg->message==WM_SYSKEYDOWN || myMsg->message==WM_SYSKEYUP)
		{
			memcpy((void *)PubMsg,(const void *)myMsg,28);
			SendMessage(g_hMainhWnd,WM_HXWDLLWX_HOOKKEY,0,0);
			memcpy((void *)myMsg,(const void *)PubMsg,28);
		}
	}
...
void setHOOK()
{
	g_hKeyboard=SetWindowsHookEx(WH_GETMESSAGE,KeyboardProc,GetModuleHandle("hxwdllwx.dll"),0);
}
