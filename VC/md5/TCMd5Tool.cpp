...
#define SN_ERROR_NULL_POINTER	 0
#define SN_ERROR_BAD_LENGTH		 1
#define SN_SUCCESS				 2
/*
...
/*初始化md5的结构*/
void TCMd5Tool::MD5Init (MD5_CTX *context)
{
...
inputLen：指定input的长度
*/
void TCMd5Tool::MD5Update(MD5_CTX *context,unsigned char * input,unsigned int  inputLen)
{
...
*/
void TCMd5Tool::MD5Final (unsigned char digest[16],MD5_CTX *context)
{
...
*/
void TCMd5Tool::MD5Transform (UINT4 state[4], unsigned char block[64])
{
...
*/
void TCMd5Tool::Encode(unsigned char *output, UINT4 *input,unsigned int  len)
{
	 unsigned int i, j; 
...
*/
void TCMd5Tool::Decode(UINT4 *output, unsigned char *input,unsigned int  len)
{
	 unsigned int i, j; 
...
int TCMd5Tool::algorithm(const char* p_pszData, const unsigned int p_nDataLen, std::string& p_strMd5)
{
	if (p_pszData == NULL)
		return SN_ERROR_NULL_POINTER;

	if (p_nDataLen <= 0)
		return SN_ERROR_BAD_LENGTH;
	...
	MD5Update(&md5, (unsigned char *)p_pszData, p_nDataLen);   //对欲加密的字符进行加密
	MD5Final(decrypt,&md5);                 //获得最终结果

	p_strMd5 = std::string((char *)decrypt);
